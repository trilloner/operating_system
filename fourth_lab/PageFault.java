...
public class PageFault {
    ...
    public static void replacePage(Vector mem, int virtPageNum, int replacePageNum, ControlPanel controlPanel) {
        ...
        int R = 0;
        int M = 0;
        int pagesCategories = 1;
        while (true) {
            oldestPage = searchPage(mem, R, M);
            if (oldestPage != -1)
                break;

            switch (pagesCategories) {
                case 1 -> {
                    R = 0;
                    M = 1;
                    pagesCategories++;
...
//        }
        removePage(mem, oldestPage, replacePageNum, controlPanel);
    }

    private static int searchPage(Vector mem, int R, int M) {
        for (int i = 0; i < mem.size(); i++) {
            Page page = (Page) mem.elementAt(i);
            if (page.physical != -1) {
                if (page.R == R && page.M == M) {
                    return page.id;
                }
            }
        }
